...
export async function minerULocalProcessing(projectId, fileName, options = {}) {
    ...
    if (url === undefined || url === null || url === '') {
      throw new Error('mineru local url configuration not found, please check if mineru local url is configured in task settings');
    }
    ...
    try{
      ...
      fs.writeFileSync(outputPath, jsonContent.md_content);
    }catch(error){
